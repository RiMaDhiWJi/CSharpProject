; (function ($, window, document, undefined) {
    ...
    var getXpath = function (xpath, xSelector, options) {
        ...
        if (options.mode === "any") {
            var tagAttr = $this.get(0).attributes;
            for (var i in tagAttr) {
                if (tagAttr.hasOwnProperty(i)) {
                    var value = tagAttr[i];
                    var jqStemp = tag + ("[" + value.name + "=\"" + value.nodeValue + "\"]");
                    if ($(jqStemp).length === 1) {
                        xpath = ("/" + tag + "[@" + value.name + "=\"" + value.nodeValue + "\"]") + xpath;
                        return "/" + xpath;
                    }
                }
            }
        }
        if (options.mode.indexOf("id") > -1 && $this.attr("id") && $(tag + "[id=\"" + $this.attr("id") + "\"]").length === 1) {
            xpath = ("/" + tag + "[@id=\"" + $this.attr("id") + "\"]") + xpath;
            return "/" + xpath;
        }
        if (options.mode.indexOf("class") > -1 && $this.attr("class")) {
    ...
    var methods = {
        ...
        getXpath: function (options) {
            ...
            if (!settings.keepTbodys) {
                xpath = xpath.replace(/\/tbody[0-9\[\]^/]*\//g, "//");
            }
            if (settings.isHightLight && settings.mode !== "any") {
